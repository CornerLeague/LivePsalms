...
- **The B4 mobile check.** ⚠️ **Mostly done, and smaller than it looks.** The layout half ran at an emulated 375×812 on 2026-08-07 and passed: the overlay covers the tab bar (hit test at the tab bar's own centre lands inside the overlay), the header does not overflow at the worst case (long book name + verse toggle + back + close), the passage label truncates rather than pushing controls off, and all controls are 44px. What is left needs **real hardware**: whether the notch/home-indicator clearance looks right — the safe-area rule is proven *wired*, since `calc(6px + env(…))` computes to `6px` rather than being dropped, but `env()` is `0` in emulation — and **whether the keyboard covers the seeded draft**, which no viewport resize can reach. The iOS Simulator is not an option on this machine: Xcode is installed but has no iOS runtime.

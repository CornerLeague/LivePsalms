...
- **The B4 mobile check.** ⚠️ **Mostly done, and smaller than it looks.** The layout half ran at an emulated 375×812 on 2026-08-07 and passed: the overlay covers the tab bar (hit test at the tab bar's own centre lands inside the overlay), the header does not overflow at the worst case (long book name + verse toggle + back + close), the passage label truncates rather than pushing controls off, and every interactive control in the header measures at least 44px (back 90×44, *Whole chapter* 72×44, close 44×44 — each measured individually, after a first pass that measured containers instead). What is left needs **a device and a signed-in session**, both: whether the notch/home-indicator clearance looks right — the safe-area rule is proven *wired*, since `calc(6px + env(…))` computes to `6px` rather than being dropped, but `env()` is `0` in emulation — and **whether the keyboard covers the seeded draft**, which no viewport resize can reach — and which needs the login, because the draft only arrives via a seeded prompt and footers render only for `userId != null`. The iOS Simulator is not an option on this machine: Xcode is installed but has no iOS runtime.
